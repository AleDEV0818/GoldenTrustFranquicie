/**
 * Dashboard Televisor - Paneles, métricas y ticker 
 * ---------------------------------------------------------------
 * Script principal para el dashboard tipo "televisor" de métricas y ranking
 * de franquicia o sucursal. Incluye:
 * - Paneles de métricas para hoy y mes (ubicación y compañía)
 * - Tablas y totales CSR para hoy y mes
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    /**
     * Convierte un string de moneda a número (soporta 1.2M, 980K, etc.)
     */
    ...
    /**
     * Formatea premium como moneda natural ($1,356,957), sin decimales.
     */
    ...
    /**
     * Actualiza la tabla CSR (hoy/mes) con los datos, mostrando máximos 10 filas.
     */
    function safeUpdateCSRTable(tableElement, csrData) {
        ...
        dataToShow.forEach(csr => {
            ...
            if (csr) {
                ...
                policiesCell.textContent = formatInteger(csr.policies || 0);
                policiesCell.className = 'csr-policies';
                const premiumCell = row.insertCell(2);
                premiumCell.textContent = formatTableCurrency(csr.premium) || '$0';
    ...
    /**
     * Actualiza los totales CSR (hoy y mes) en el dashboard.
     */
    ...
    /**
     * Agrega y remueve animación visual a métricas cambiantes.
     */
    ...
    /**
     * Aplica animación "changing" a todos los valores métricos y totales CSR.
     */
    ...
    /**
     * Reinicia y actualiza el contador de próxima actualización (segundos).
     */
    ...
    /**
     * Inicializa el valor del goal mensual desde el backend y lo muestra formateado.
     */
    function initMonthlyGoalFromData(data) {
        try {
            let rawGoal = data?.monthly_goal;
            monthlyGoal = parseCurrencyToNumber(rawGoal);
            if (!monthlyGoal || isNaN(monthlyGoal)) monthlyGoal = 10000000;
            const formattedGoal = formatNaturalCurrency(monthlyGoal);
            if (elements.monthlyGoalAmount)
                elements.monthlyGoalAmount.textContent = formattedGoal;
            if (elements.monthlyGoalRaw)
                elements.monthlyGoalRaw.textContent = monthlyGoal;
        } catch (error) {
            monthlyGoal = 10000000;
            if (elements.monthlyGoalAmount)
                elements.monthlyGoalAmount.textContent = formatNaturalCurrency(10000000);
            if (elements.monthlyGoalRaw)
                elements.monthlyGoalRaw.textContent = 10000000;
        }
    }
    ...
    /**
     * Sincroniza el reloj para actualizar justo al inicio de cada minuto.
     */
    ...
    /**
     * Actualiza el texto de los alias de ubicación en el dashboard.
     */
    ...
    /**
     * Actualiza toda la UI del dashboard con los datos recibidos del backend.
     */
    function updateUI(data) {
        // Inicializa la meta mensual
        initMonthlyGoalFromData(data);

        // Actualiza métricas principales y aplica animación
        ...
        // Actualiza y anima la meta mensual (goal)
        try {
        ...
        // Actualiza tablas y totales CSR
        ...
        // Reinicia contador de próxima actualización
        safeResetNextUpdateCounter();

        // Actualiza alias de ubicación
        updateLocationAliasInDOM(data.locationAlias || 'Corporate', data.locationType);
    ...
    /**
     * Crea elementos de fondo visual animados en el dashboard.
     */
    ...
    /**
     * Lee los parámetros de la URL relevantes para la consulta.
    ...
    /**
     * Llama al backend y actualiza la UI con los datos.
    ...
    /**
     * Verifica que los elementos críticos del dashboard existen en el DOM.
     * Si falta alguno, muestra error crítico.
     */
