...
--
-- Name: entra; Type: SCHEMA; Schema: -; Owner: postgres
--

CREATE SCHEMA entra;


ALTER SCHEMA entra OWNER TO postgres;
...
--
-- Name: qq; Type: SCHEMA; Schema: -; Owner: postgres
--

CREATE SCHEMA qq;


ALTER SCHEMA qq OWNER TO postgres;
...
CREATE FUNCTION intranet.franchise_cache_meta(p_date date, p_stale_minutes integer DEFAULT 15) RETURNS TABLE(requested_date date, used_date date, as_of timestamp with time zone, is_stale boolean, refreshing boolean)
  ...
  WITH used AS (
    SELECT COALESCE(
             (SELECT max(date1) FROM entra.franchise_cache WHERE date1 <= p_date),
             (SELECT max(date1) FROM entra.franchise_cache)
           ) AS use_date
  ),
  agg AS (
    SELECT u.use_date, max(c.updated_at) AS as_of
    FROM used u
    LEFT JOIN entra.franchise_cache c ON c.date1 = u.use_date
    GROUP BY u.use_date
  ),
  job AS (
    SELECT EXISTS (
      SELECT 1 FROM intranet.kpi_refresh_jobs
      WHERE date1 = p_date AND status IN ('queued','running')
    ) AS refreshing
  )
  SELECT
    p_date AS requested_date,
    a.use_date AS used_date,
    a.as_of,
    CASE WHEN a.as_of IS NULL THEN true
         ELSE now() - a.as_of > make_interval(mins => p_stale_minutes) END AS is_stale,
    j.refreshing
  FROM agg a, job j;
$$;
...
CREATE FUNCTION intranet.franchise_error_totals(p_location_id integer, p_start_date date DEFAULT NULL::date, p_end_date date DEFAULT NULL::date) RETURNS TABLE(location_id integer, franchise text, binder_errors integer, csr_total integer, producer_total integer, missing_any_total integer, total_errors integer, start_date date, end_date date)
    LANGUAGE sql
    AS $$
    WITH param AS (
        -- Si el location_id no existe o es NULL, asumimos tipo 1 (mostrar todas las franquicias)
        SELECT COALESCE(
    ...
    SELECT
        ...
        COALESCE(bc.cnt, 0) AS binder_errors,
        COALESCE(cp.csr_total, 0) AS csr_total,
        COALESCE(cp.producer_total, 0) AS producer_total,
        COALESCE(cp.missing_any_total, 0) AS missing_any_total,
        COALESCE(bc.cnt, 0) + COALESCE(cp.missing_any_total, 0) AS total_errors,
        p_start_date AS start_date,
        p_end_date   AS end_date
    FROM qq.locations l
    CROSS JOIN param
    -- Errores por binder (usa la función de binder con rango de fechas)
    LEFT JOIN LATERAL (
        SELECT COUNT(*)::int AS cnt
        FROM intranet.get_policy_report_by_location(l.location_id, p_start_date, p_end_date)
    ) bc ON TRUE
    -- Errores por CSR y por Producer (separados) y el total único (sin doble conteo)
    LEFT JOIN LATERAL (
        SELECT
    ...
    ORDER BY (COALESCE(bc.cnt, 0) + COALESCE(cp.missing_any_total, 0)) DESC, franchise;
$$;
...
CREATE FUNCTION intranet.franchise_kpis_totals(date1 date) RETURNS TABLE(premium_and_policies text, renewed_and_policies text, lost_and_policies text, percent_and_max text)
  ...
  WITH pick AS (
    SELECT
      COALESCE(
        (SELECT max(date1) FROM entra.franchise_cache WHERE date1 <= $1),
        (SELECT max(date1) FROM entra.franchise_cache)
      ) AS use_date
  ),
  agg AS (
    ...
    FROM pick p
    LEFT JOIN entra.franchise_cache c
      ON c.date1 = p.use_date
  ),
  perc AS (
    SELECT
      CASE WHEN a.total_policies = 0 THEN 0.0
...
CREATE FUNCTION intranet.get_codes_by_location(p_location_id integer) RETURNS TABLE(agency character varying, company character varying, code character varying, login character varying, password character varying)
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_location_type integer;
...
BEGIN
    SELECT location_type, alias INTO v_location_type, v_agency_alias
    FROM qq.locations
    WHERE location_id = p_location_id;
    ...
    IF v_location_type = 1 THEN
        -- Solo agencias corporativas que SÍ tienen códigos ACTIVOS
        RETURN QUERY
        SELECT 
            l.alias as agency,
            c.company,
...
BEGIN
  IF p_full THEN
    DELETE FROM entra.franchise_cache WHERE date1 = p_date;
  END IF;

  INSERT INTO entra.franchise_cache (
  ...
  ON CONFLICT (date1, location_id) DO UPDATE SET
    franchise        = EXCLUDED.franchise,
    premium          = EXCLUDED.premium,
    policies         = EXCLUDED.policies,
    renewed_premium  = EXCLUDED.renewed_premium,
    renewed_policies = EXCLUDED.renewed_policies,
    lost_premium     = EXCLUDED.lost_premium,
    lost_policies    = EXCLUDED.lost_policies,
    renewed_percent  = EXCLUDED.renewed_percent,
...
--
-- Name: refresh_franchise_cache_merge(date); Type: FUNCTION; Schema: intranet; Owner: postgres
--

CREATE FUNCTION intranet.refresh_franchise_cache_merge(p_date date) RETURNS void
    LANGUAGE plpgsql
    AS $$
BEGIN
  -- Tabla derivada con el origen
  WITH src AS (
    SELECT
      p_date AS date1,
      s.location_id,
      s.franchise,
      COALESCE(NULLIF(replace(s.premium,           ',', ''), '')::numeric, 0) AS premium,
      COALESCE(NULLIF(replace(s.policies,          ',', ''), '')::bigint,  0) AS policies,
      COALESCE(NULLIF(replace(s.renewed_premium,   ',', ''), '')::numeric, 0) AS renewed_premium,
      COALESCE(NULLIF(replace(s.renewed_policies,  ',', ''), '')::bigint,  0) AS renewed_policies,
      COALESCE(NULLIF(replace(s.lost_premium,      ',', ''), '')::numeric, 0) AS lost_premium,
      COALESCE(NULLIF(replace(s.lost_policies,     ',', ''), '')::bigint,  0) AS lost_policies,
      COALESCE(NULLIF(regexp_replace(s.renewed_percent, '[^0-9.\-]', '', 'g'), '')::numeric, 0) AS renewed_percent,
      COALESCE(NULLIF(regexp_replace(s.max_percent,      '[^0-9.\-]', '', 'g'), '')::numeric, 0) AS max_percent
    FROM intranet.franchise_kpis_dashboard(p_date) s
  )
  MERGE INTO entra.franchise_cache c
  USING src s
    ON (c.date1 = s.date1 AND c.location_id = s.location_id)
  WHEN MATCHED THEN UPDATE SET
    franchise        = s.franchise,
    premium          = s.premium,
    policies         = s.policies,
    renewed_premium  = s.renewed_premium,
    renewed_policies = s.renewed_policies,
    lost_premium     = s.lost_premium,
    lost_policies    = s.lost_policies,
    renewed_percent  = s.renewed_percent,
    max_percent      = s.max_percent,
    updated_at       = now()
  WHEN NOT MATCHED THEN INSERT (
    date1, location_id, franchise, premium, policies,
    renewed_premium, renewed_policies, lost_premium, lost_policies,
    renewed_percent, max_percent, updated_at
  ) VALUES (
    s.date1, s.location_id, s.franchise, s.premium, s.policies,
    s.renewed_premium, s.renewed_policies, s.lost_premium, s.lost_policies,
    s.renewed_percent, s.max_percent, now()
  );

  -- Borra las filas  de las fecha que no están en src
  DELETE FROM entra.franchise_cache c
  WHERE c.date1 = p_date
    AND NOT EXISTS (
      SELECT 1 FROM intranet.franchise_kpis_dashboard(p_date) s
...
ALTER FUNCTION intranet.refresh_franchise_cache_merge(p_date date) OWNER TO postgres;
...
--
-- Name: get_policies_missing_csr_or_producer(integer); Type: FUNCTION; Schema: qq; Owner: postgres
--

CREATE FUNCTION qq.get_policies_missing_csr_or_producer(p_location_id integer) RETURNS TABLE(policy_number character varying, carrier text, line_of_business text, csr text, producer text, location text, business_type character varying, binder_date date, effective_date date, missing_fields text)
    LANGUAGE sql
    AS $$
    SELECT
        p.policy_number,
        carr.carrier_name AS carrier,
        lob.display_name AS line_of_business,
        COALESCE(
            ucsr.display_name,
            CONCAT_WS(' ', ucsr.first_name, ucsr.last_name),
        ...
        p.binder_date,
        p.effective_date,
        -- Indica qué campo(s) faltan: "CSR", "Producer" o "CSR, Producer"
        concat_ws(', ',
            CASE WHEN p.csr_id IS NULL THEN 'CSR' END,
            CASE WHEN p.producer_ids IS NULL OR COALESCE(array_length(p.producer_ids, 1), 0) = 0 THEN 'Producer' END
        ) AS missing_fields
    FROM qq.policies AS p
    -- Agente -> de ahí sacamos location_id
    JOIN qq.contacts AS a
      ON a.entity_id = p.agent_id
    LEFT JOIN qq.locations AS loc
      ON loc.location_id = a.location_id
    -- Carrier name (evita duplicados usando subconsultas)
    LEFT JOIN LATERAL (
        SELECT COALESCE(
            (SELECT hc1.name FROM qq.head_carriers hc1 WHERE hc1.head_carrier_id = p.carrier_id LIMIT 1),
            (SELECT hc2.name FROM qq.head_carriers hc2 WHERE hc2.contact_id      = p.carrier_id LIMIT 1)
        ) AS carrier_name
    ) carr ON true
    -- LOB
    LEFT JOIN qq.lob AS lob
    ...
    LEFT JOIN qq.contacts AS ucsr
      ON ucsr.entity_id = p.csr_id
    -- Producers desde contacts (maneja NULL y arreglo vacío)
    LEFT JOIN LATERAL (
        SELECT string_agg(
                   COALESCE(u.display_name,
                            CONCAT_WS(' ', u.first_name, u.last_name),
...
ALTER FUNCTION qq.get_policies_missing_csr_or_producer(p_location_id integer) OWNER TO postgres;

--
-- Name: get_policy_report_by_location(integer); Type: FUNCTION; Schema: qq; Owner: postgres
--

CREATE FUNCTION qq.get_policy_report_by_location(p_location_id integer) RETURNS TABLE(policy_number character varying, carrier text, line_of_business text, csr text, producer text, location text, business_type character varying, binder_date date, effective_date date)
    LANGUAGE sql
    AS $$
    SELECT
        p.policy_number,
        COALESCE(hc_id.name, hc_contact.name) AS carrier,
        lob.display_name AS line_of_business,
        COALESCE(ucsr.display_name,
                 CONCAT_WS(' ', ucsr.first_name, ucsr.last_name),
                 ucsr.business_name) AS csr,
        ...
        p.effective_date
    FROM qq.policies AS p
    -- El agente de la póliza vive en qq.contacts; de ahí sacamos el location_id
    JOIN qq.contacts AS a
      ON a.entity_id = p.agent_id
    LEFT JOIN qq.locations AS loc
      ON loc.location_id = a.location_id
    -- Carrier: intentamos por head_carrier_id y también por contact_id (por si el id refiere a contacto)
    LEFT JOIN qq.head_carriers AS hc_id
      ON hc_id.head_carrier_id = p.carrier_id
    LEFT JOIN qq.head_carriers AS hc_contact
      ON hc_contact.contact_id = p.carrier_id
    -- LOB
    LEFT JOIN qq.lob AS lob
      ON lob.lob_id = p.lob_id
    -- CSR desde contacts
    LEFT JOIN qq.contacts AS ucsr
      ON ucsr.entity_id = p.csr_id
    -- Producers desde contacts (arreglo de IDs en policies.producer_ids)
    LEFT JOIN LATERAL (
        SELECT string_agg(
                   COALESCE(u.display_name,
                            CONCAT_WS(' ', u.first_name, u.last_name),
        ...
        FROM unnest(p.producer_ids) AS pid
        JOIN qq.contacts u ON u.entity_id = pid
    ) prod ON true
    WHERE a.location_id = p_location_id
      AND (
...
ALTER FUNCTION qq.get_policy_report_by_location(p_location_id integer) OWNER TO postgres;
...
--
-- Name: carriers_trans_types; Type: TABLE; Schema: admin; Owner: postgres
--

CREATE TABLE admin.carriers_trans_types (
    carrier_id integer NOT NULL,
    carrier_trans_type character varying NOT NULL,
    trans_type_id smallint NOT NULL
);


ALTER TABLE admin.carriers_trans_types OWNER TO postgres;

--
-- Name: code; Type: TABLE; Schema: admin; Owner: postgres
--
...
ALTER TABLE admin.code OWNER TO postgres;

--
-- Name: crypto; Type: TABLE; Schema: admin; Owner: postgres
--

CREATE TABLE admin.crypto (
...
ALTER TABLE admin.crypto OWNER TO postgres;

--
-- Name: form_of_distribution; Type: TABLE; Schema: admin; Owner: postgres
--

CREATE TABLE admin.form_of_distribution (
    form_id smallint NOT NULL,
    display_name character varying NOT NULL
);


ALTER TABLE admin.form_of_distribution OWNER TO postgres;

--
-- Name: form_of_distribution_form_id_seq; Type: SEQUENCE; Schema: admin; Owner: postgres
--

CREATE SEQUENCE admin.form_of_distribution_form_id_seq
    AS smallint
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
...
ALTER SEQUENCE admin.form_of_distribution_form_id_seq OWNER TO postgres;

--
-- Name: form_of_distribution_form_id_seq; Type: SEQUENCE OWNED BY; Schema: admin; Owner: postgres
--

ALTER SEQUENCE admin.form_of_distribution_form_id_seq OWNED BY admin.form_of_distribution.form_id;


--
-- Name: licensee_detail; Type: TABLE; Schema: admin; Owner: postgres
--

CREATE TABLE admin.licensee_detail (
    licensee_detail_id integer NOT NULL,
...
ALTER TABLE admin.licensee_detail OWNER TO postgres;

--
-- Name: licensee_detail_licensee_detail_id_seq; Type: SEQUENCE; Schema: admin; Owner: postgres
--

CREATE SEQUENCE admin.licensee_detail_licensee_detail_id_seq
    AS integer
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
...
ALTER SEQUENCE admin.licensee_detail_licensee_detail_id_seq OWNER TO postgres;

--
-- Name: licensee_detail_licensee_detail_id_seq; Type: SEQUENCE OWNED BY; Schema: admin; Owner: postgres
--

ALTER SEQUENCE admin.licensee_detail_licensee_detail_id_seq OWNED BY admin.licensee_detail.licensee_detail_id;


--
-- Name: lob; Type: TABLE; Schema: admin; Owner: postgres
--

CREATE TABLE admin.lob (
    display_name character varying NOT NULL,
    id_line smallint NOT NULL,
    lob_id smallint NOT NULL,
    line character varying
);


ALTER TABLE admin.lob OWNER TO postgres;

--
-- Name: location_types; Type: TABLE; Schema: admin; Owner: postgres
--

CREATE TABLE admin.location_types (
    location_type_id smallint NOT NULL,
    location_type character varying NOT NULL
);


ALTER TABLE admin.location_types OWNER TO postgres;

--
-- Name: mail; Type: TABLE; Schema: admin; Owner: postgres
--

CREATE TABLE admin.mail (
...
ALTER TABLE admin.mail OWNER TO postgres;

--
-- Name: periods; Type: TABLE; Schema: admin; Owner: postgres
--

CREATE TABLE admin.periods (
...
ALTER TABLE admin.periods OWNER TO postgres;

--
-- Name: transaction_types; Type: TABLE; Schema: admin; Owner: postgres
--

CREATE TABLE admin.transaction_types (
    trans_type_id smallint NOT NULL,
    display_name character varying NOT NULL
);


ALTER TABLE admin.transaction_types OWNER TO postgres;

--
-- Name: transaction_types_trans_type_id_seq; Type: SEQUENCE; Schema: admin; Owner: postgres
--

CREATE SEQUENCE admin.transaction_types_trans_type_id_seq
    AS smallint
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
...
ALTER SEQUENCE admin.transaction_types_trans_type_id_seq OWNER TO postgres;

--
-- Name: transaction_types_trans_type_id_seq; Type: SEQUENCE OWNED BY; Schema: admin; Owner: postgres
--

ALTER SEQUENCE admin.transaction_types_trans_type_id_seq OWNED BY admin.transaction_types.trans_type_id;


--
-- Name: franchise_cache; Type: TABLE; Schema: entra; Owner: postgres
--

CREATE TABLE entra.franchise_cache (
    date1 date NOT NULL,
...
ALTER TABLE entra.franchise_cache OWNER TO postgres;

--
-- Name: goals; Type: TABLE; Schema: entra; Owner: postgres
--

CREATE TABLE entra.goals (
    id integer NOT NULL,
...
ALTER TABLE entra.goals OWNER TO postgres;

--
-- Name: goals_id_seq; Type: SEQUENCE; Schema: entra; Owner: postgres
--

CREATE SEQUENCE entra.goals_id_seq
    AS integer
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
...
ALTER SEQUENCE entra.goals_id_seq OWNER TO postgres;

--
-- Name: goals_id_seq; Type: SEQUENCE OWNED BY; Schema: entra; Owner: postgres
--

ALTER SEQUENCE entra.goals_id_seq OWNED BY entra.goals.id;


--
-- Name: licensed; Type: TABLE; Schema: entra; Owner: postgres
--

CREATE TABLE entra.licensed (
...
ALTER TABLE entra.licensed OWNER TO postgres;

--
-- Name: TABLE licensed; Type: COMMENT; Schema: entra; Owner: postgres
--

COMMENT ON TABLE entra.licensed IS 'Agentes con licencia para vender pólizas de seguros';


--
-- Name: COLUMN licensed.license_number; Type: COMMENT; Schema: entra; Owner: postgres
--

COMMENT ON COLUMN entra.licensed.license_number IS 'Número de licencia del agente';


--
-- Name: COLUMN licensed.issue_date; Type: COMMENT; Schema: entra; Owner: postgres
--

COMMENT ON COLUMN entra.licensed.issue_date IS 'Fecha de emisión de la licencia';


--
-- Name: COLUMN licensed.exp_date; Type: COMMENT; Schema: entra; Owner: postgres
--

COMMENT ON COLUMN entra.licensed.exp_date IS 'Fecha de expiración de la licencia';


--
-- Name: COLUMN licensed.active; Type: COMMENT; Schema: entra; Owner: postgres
--

COMMENT ON COLUMN entra.licensed.active IS 'Estado de la licencia: true=activo, false=inactivo';


--
-- Name: COLUMN licensed.location_id; Type: COMMENT; Schema: entra; Owner: postgres
--

COMMENT ON COLUMN entra.licensed.location_id IS 'ID de la ubicación del agente (relacionada con la tabla qq.locations)';


--
-- Name: COLUMN licensed.updated_by_display_name; Type: COMMENT; Schema: entra; Owner: postgres
--

COMMENT ON COLUMN entra.licensed.updated_by_display_name IS 'Nombre visible del usuario que realizó la última operación (inserción o actualización)';


--
-- Name: COLUMN licensed.updated_by_user_id; Type: COMMENT; Schema: entra; Owner: postgres
--

COMMENT ON COLUMN entra.licensed.updated_by_user_id IS 'ID del usuario que realizó la última operación (inserción o actualización)';


--
-- Name: licensed_licensed_agent_id_seq; Type: SEQUENCE; Schema: entra; Owner: postgres
--

ALTER TABLE entra.licensed ALTER COLUMN licensed_agent_id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME entra.licensed_licensed_agent_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
--
-- Name: mail; Type: TABLE; Schema: entra; Owner: postgres
--

CREATE TABLE entra.mail (
    user_id character varying NOT NULL,
    display_name character varying,
    mail character varying,
    location_id integer
...
ALTER TABLE entra.mail OWNER TO postgres;

--
-- Name: users; Type: TABLE; Schema: entra; Owner: postgres
--

CREATE TABLE entra.users (
    user_id character varying NOT NULL,
    display_name character varying,
...
ALTER TABLE entra.users OWNER TO postgres;

--
-- Name: contacts; Type: TABLE; Schema: qq; Owner: postgres
--

CREATE TABLE qq.contacts (
    ...
    display_name character varying,
    type_display character varying(1),
    ...
    contact_type character varying(1),
    head_comp character varying
);


ALTER TABLE qq.contacts OWNER TO postgres;

--
-- Name: locations; Type: TABLE; Schema: qq; Owner: postgres
--

CREATE TABLE qq.locations (
    location_id integer NOT NULL,
    location_name character varying,
    is_default boolean,
    qq_id character varying,
    alias character varying,
    street_address_line1 character varying,
    street_address_line2 character varying,
    city character varying,
    ...
    default_state_code character varying,
    business_fein character varying,
    location_type smallint,
    date_last_modified timestamp with time zone,
    comm boolean,
    royalty_fee numeric(4,3),
    comm_percent numeric(3,2)
);


ALTER TABLE qq.locations OWNER TO postgres;

--
-- Name: policies; Type: TABLE; Schema: qq; Owner: postgres
--

CREATE TABLE qq.policies (
    ...
    lob_id integer,
    binder_date date,
    business_type character varying(15),
    policy_status character varying(15),
...
ALTER TABLE qq.policies OWNER TO postgres;
...
SELECT
    NULL::character varying AS csr,
    NULL::bigint AS policies,
    NULL::text AS premium,
...
--
-- Name: lob; Type: TABLE; Schema: qq; Owner: postgres
--
...
ALTER TABLE qq.lob OWNER TO postgres;
...
--
-- Name: adjusments; Type: TABLE; Schema: qq; Owner: postgres
--

CREATE TABLE qq.adjusments (
    adjusment_id integer NOT NULL,
    policy_id integer NOT NULL,
    adjusment_date timestamp without time zone,
    adjusment_type character varying,
    premium_change money,
    csr_id integer
);


ALTER TABLE qq.adjusments OWNER TO postgres;

--
-- Name: business_types; Type: TABLE; Schema: qq; Owner: postgres
--

CREATE TABLE qq.business_types (
    business_type_id character varying(1) NOT NULL,
    display_name character varying NOT NULL
);


ALTER TABLE qq.business_types OWNER TO postgres;

--
-- Name: customer_info; Type: TABLE; Schema: qq; Owner: postgres
--

CREATE TABLE qq.customer_info (
    entity_id integer NOT NULL,
    dob date,
    gender character varying,
    driver_license_no character varying,
    customer_source_id smallint,
    customer_source character varying,
    source_detail character varying
);


ALTER TABLE qq.customer_info OWNER TO postgres;

--
-- Name: head_carriers; Type: TABLE; Schema: qq; Owner: postgres
--

CREATE TABLE qq.head_carriers (
    head_carrier_id integer NOT NULL,
    name character varying NOT NULL,
    contact_id integer NOT NULL
);


ALTER TABLE qq.head_carriers OWNER TO postgres;

--
-- Name: head_carriers_head_carrier_id_seq; Type: SEQUENCE; Schema: qq; Owner: postgres
--

CREATE SEQUENCE qq.head_carriers_head_carrier_id_seq
    AS integer
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
...
ALTER SEQUENCE qq.head_carriers_head_carrier_id_seq OWNER TO postgres;

--
-- Name: head_carriers_head_carrier_id_seq; Type: SEQUENCE OWNED BY; Schema: qq; Owner: postgres
--

ALTER SEQUENCE qq.head_carriers_head_carrier_id_seq OWNED BY qq.head_carriers.head_carrier_id;


--
-- Name: type_displays; Type: TABLE; Schema: qq; Owner: postgres
--

CREATE TABLE qq.type_displays (
    type_display character varying(1) NOT NULL,
    name character varying NOT NULL
);


ALTER TABLE qq.type_displays OWNER TO postgres;

--
-- Name: form_of_distribution form_id; Type: DEFAULT; Schema: admin; Owner: postgres
--

ALTER TABLE ONLY admin.form_of_distribution ALTER COLUMN form_id SET DEFAULT nextval('admin.form_of_distribution_form_id_seq'::regclass);


--
-- Name: licensee_detail licensee_detail_id; Type: DEFAULT; Schema: admin; Owner: postgres
--

ALTER TABLE ONLY admin.licensee_detail ALTER COLUMN licensee_detail_id SET DEFAULT nextval('admin.licensee_detail_licensee_detail_id_seq'::regclass);


--
-- Name: transaction_types trans_type_id; Type: DEFAULT; Schema: admin; Owner: postgres
--

ALTER TABLE ONLY admin.transaction_types ALTER COLUMN trans_type_id SET DEFAULT nextval('admin.transaction_types_trans_type_id_seq'::regclass);


--
-- Name: goals id; Type: DEFAULT; Schema: entra; Owner: postgres
--

ALTER TABLE ONLY entra.goals ALTER COLUMN id SET DEFAULT nextval('entra.goals_id_seq'::regclass);
...
--
-- Name: head_carriers head_carrier_id; Type: DEFAULT; Schema: qq; Owner: postgres
--

ALTER TABLE ONLY qq.head_carriers ALTER COLUMN head_carrier_id SET DEFAULT nextval('qq.head_carriers_head_carrier_id_seq'::regclass);


--
-- Name: periods actual_period_pkey; Type: CONSTRAINT; Schema: admin; Owner: postgres
--

ALTER TABLE ONLY admin.periods
    ADD CONSTRAINT actual_period_pkey PRIMARY KEY (period);


--
-- Name: carriers_trans_types carriers_trans_types_pkey; Type: CONSTRAINT; Schema: admin; Owner: postgres
--

ALTER TABLE ONLY admin.carriers_trans_types
    ADD CONSTRAINT carriers_trans_types_pkey PRIMARY KEY (carrier_id, carrier_trans_type);


--
-- Name: code code_pkey; Type: CONSTRAINT; Schema: admin; Owner: postgres
--

ALTER TABLE ONLY admin.code
    ADD CONSTRAINT code_pkey PRIMARY KEY (agency, company, code);


--
-- Name: form_of_distribution form_of_distribution_pkey; Type: CONSTRAINT; Schema: admin; Owner: postgres
--

ALTER TABLE ONLY admin.form_of_distribution
    ADD CONSTRAINT form_of_distribution_pkey PRIMARY KEY (form_id);


--
-- Name: licensee_detail licensee_detail_pkey; Type: CONSTRAINT; Schema: admin; Owner: postgres
--

ALTER TABLE ONLY admin.licensee_detail
    ADD CONSTRAINT licensee_detail_pkey PRIMARY KEY (licensee_detail_id);


--
-- Name: lob lob_pkey; Type: CONSTRAINT; Schema: admin; Owner: postgres
--

ALTER TABLE ONLY admin.lob
    ADD CONSTRAINT lob_pkey PRIMARY KEY (lob_id);


--
-- Name: location_types location_types_pkey; Type: CONSTRAINT; Schema: admin; Owner: postgres
--

ALTER TABLE ONLY admin.location_types
    ADD CONSTRAINT location_types_pkey PRIMARY KEY (location_type_id);


--
-- Name: mail mail_pkey; Type: CONSTRAINT; Schema: admin; Owner: postgres
--

ALTER TABLE ONLY admin.mail
    ADD CONSTRAINT mail_pkey PRIMARY KEY (user_id);


--
-- Name: transaction_types transaction_types_pkey; Type: CONSTRAINT; Schema: admin; Owner: postgres
--

ALTER TABLE ONLY admin.transaction_types
    ADD CONSTRAINT transaction_types_pkey PRIMARY KEY (trans_type_id);


--
-- Name: users entra_users_pkey; Type: CONSTRAINT; Schema: entra; Owner: postgres
--

ALTER TABLE ONLY entra.users
    ADD CONSTRAINT entra_users_pkey PRIMARY KEY (user_id);


--
-- Name: franchise_cache franchise_cache_pkey; Type: CONSTRAINT; Schema: entra; Owner: postgres
--

ALTER TABLE ONLY entra.franchise_cache
    ADD CONSTRAINT franchise_cache_pkey PRIMARY KEY (date1, location_id);


--
-- Name: goals goals_pkey; Type: CONSTRAINT; Schema: entra; Owner: postgres
--

ALTER TABLE ONLY entra.goals
    ADD CONSTRAINT goals_pkey PRIMARY KEY (id);


--
-- Name: licensed licensed_pkey; Type: CONSTRAINT; Schema: entra; Owner: postgres
--

ALTER TABLE ONLY entra.licensed
    ADD CONSTRAINT licensed_pkey PRIMARY KEY (licensed_agent_id);


--
-- Name: mail mail_pkey; Type: CONSTRAINT; Schema: entra; Owner: postgres
--

ALTER TABLE ONLY entra.mail
    ADD CONSTRAINT mail_pkey PRIMARY KEY (user_id);
...
--
-- Name: adjusments Adjusment_pkey; Type: CONSTRAINT; Schema: qq; Owner: postgres
--

ALTER TABLE ONLY qq.adjusments
    ADD CONSTRAINT "Adjusment_pkey" PRIMARY KEY (adjusment_id);


--
-- Name: business_types business_type_pkey; Type: CONSTRAINT; Schema: qq; Owner: postgres
--

ALTER TABLE ONLY qq.business_types
    ADD CONSTRAINT business_type_pkey PRIMARY KEY (business_type_id);


--
-- Name: customer_info contacts_information_pkey; Type: CONSTRAINT; Schema: qq; Owner: postgres
--

ALTER TABLE ONLY qq.customer_info
    ADD CONSTRAINT contacts_information_pkey PRIMARY KEY (entity_id);


--
-- Name: contacts contacts_pkey; Type: CONSTRAINT; Schema: qq; Owner: postgres
--

ALTER TABLE ONLY qq.contacts
    ADD CONSTRAINT contacts_pkey PRIMARY KEY (entity_id);


--
-- Name: head_carriers head_carriers_pkey; Type: CONSTRAINT; Schema: qq; Owner: postgres
--

ALTER TABLE ONLY qq.head_carriers
    ADD CONSTRAINT head_carriers_pkey PRIMARY KEY (contact_id);


--
-- Name: lob lob_pkey; Type: CONSTRAINT; Schema: qq; Owner: postgres
--

ALTER TABLE ONLY qq.lob
    ADD CONSTRAINT lob_pkey PRIMARY KEY (lob_id);


--
-- Name: locations locations_pkey; Type: CONSTRAINT; Schema: qq; Owner: postgres
--

ALTER TABLE ONLY qq.locations
    ADD CONSTRAINT locations_pkey PRIMARY KEY (location_id);


--
-- Name: policies qq_policies_pkey; Type: CONSTRAINT; Schema: qq; Owner: postgres
--

ALTER TABLE ONLY qq.policies
    ADD CONSTRAINT qq_policies_pkey PRIMARY KEY (policy_id);


--
-- Name: type_displays type_displays_pkey; Type: CONSTRAINT; Schema: qq; Owner: postgres
--

ALTER TABLE ONLY qq.type_displays
    ADD CONSTRAINT type_displays_pkey PRIMARY KEY (type_display);


--
-- Name: franchise_cache_date1_idx; Type: INDEX; Schema: entra; Owner: postgres
--

CREATE INDEX franchise_cache_date1_idx ON entra.franchise_cache USING btree (date1);
...
--
-- Name: agency_csr_last_week _RETURN; Type: RULE; Schema: intranet; Owner: postgres
--
...
--
-- Name: licensed licensed_user_id_fkey; Type: FK CONSTRAINT; Schema: entra; Owner: postgres
--
