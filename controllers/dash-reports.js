...
const dashboard = async (req, res) => {
    ...
    const final_date = date; // <-- Hasta hoy

    // Convertir fechas a ISO para queries SQL
    const initial_date_iso = initial_date.toISOString().split('T')[0];
    const final_date_iso = final_date.toISOString().split('T')[0];
    ...
    // MES (del 1 al día actual)
    production = req.user.location_type != 1
        ? (await pool.query(`SELECT * FROM intranet.dashboard_location_month($1, $2, $3)`, [initial_date_iso, final_date_iso, req.user.location_id])).rows
        : (await pool.query(`SELECT * FROM intranet.dashboard_sales_month_total_by_type_tkg($1, $2)`, [initial_date_iso, final_date_iso])).rows;
    ...
    result = await pool.query(
        ...
        [initial_date_iso, final_date_iso, req.user.location_id]
    );

    data.csr_nb_ranking = result.rows;
    let inicial1, inicial2;
...
const dashboardMetrics = async (req, res) => {
    ...
    // Obtener métricas para el mes (del 1 hasta hoy)
    production = req.user.location_type != 1
        ? (await pool.query(`SELECT * FROM intranet.dashboard_location_month($1, $2, $3)`, [initial_date_iso, final_date_iso, req.user.location_id])).rows
        : (await pool.query(`SELECT * FROM intranet.dashboard_sales_month_total_by_type_tkg($1, $2)`, [initial_date_iso, final_date_iso])).rows;
